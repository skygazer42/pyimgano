...
- For `--for-command train`, the payload also includes structured `recipe_list_command`, `recipe_info_command`, `dry_run_command`, `preflight_command`, and `recipe_run_command` fields so automation can surface the same recipe discovery path without parsing `suggested_commands`.
...
- `--webhook-bearer-token-file PATH` resolves that bearer token from a file instead of the command line.
