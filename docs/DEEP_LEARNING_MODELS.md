...
- `vision_univad` delegates to the official UniVAD source instead of the removed
  prototype-fusion proxy. It preserves the released 448px CLIP-L/14@336,
  DINOv2-G/14 and DINO-S/8 path plus C³ component segmentation, CAPM matching
  and GECM graph aggregation. `fit` installs the normal reference images and
  calibrates a threshold; inputs must be paths with component masks generated by
