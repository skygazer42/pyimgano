...
def test_jpeg_compress_preserves_shape_and_dtype() -> None:
    img = np.random.default_rng(1).integers(0, 255, size=(32, 32, 3), dtype=np.uint8)
...
def test_vignette_runs() -> None:
    img = np.random.default_rng(2).integers(0, 255, size=(32, 32, 3), dtype=np.uint8)
...
def test_random_channel_gain_runs() -> None:
    img = np.random.default_rng(3).integers(0, 255, size=(16, 16, 3), dtype=np.uint8)
...
def test_industrial_camera_robust_preset_is_callable() -> None:
    ...
    img = np.random.default_rng(4).integers(0, 255, size=(32, 32, 3), dtype=np.uint8)
    out = aug(img)
    assert out.shape == img.shape
    assert out.dtype == np.uint8
...
def test_industrial_drift_preset_is_callable() -> None:
    ...
    img = np.random.default_rng(5).integers(0, 255, size=(32, 32, 3), dtype=np.uint8)
    out = aug(img)
    assert out.shape == img.shape
    assert out.dtype == np.uint8
...
def test_industrial_surface_defect_synthesis_preset_is_callable() -> None:
    ...
    img = np.random.default_rng(6).integers(0, 255, size=(32, 32, 3), dtype=np.uint8)
    out = aug(img)
    assert out.shape == img.shape
    assert out.dtype == np.uint8
...
def test_industrial_white_balance_preserves_contract() -> None:
    img = np.random.default_rng(8).integers(0, 255, size=(32, 32, 3), dtype=np.uint8)
...
def test_industrial_homomorphic_filter_preserves_contract() -> None:
    img = np.random.default_rng(9).integers(0, 255, size=(32, 32, 3), dtype=np.uint8)
...
def test_apply_illumination_contrast_runs() -> None:
    img = np.random.default_rng(10).integers(0, 255, size=(32, 32, 3), dtype=np.uint8)
