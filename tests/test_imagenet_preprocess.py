...
def test_preprocess_imagenet_batch_preserves_unit_float_scale() -> None:
    pytest.importorskip("torch")

    from pyimgano.models._imagenet_preprocess import preprocess_imagenet_batch

    batch = np.ones((1, 1, 1, 3), dtype=np.float32)
    out = preprocess_imagenet_batch(batch)

    assert float(out[0, 0, 0, 0]) == pytest.approx((1.0 - 0.485) / 0.229)
