...
def test_integration_train_then_infer_infer_config_manifest(tmp_path, capsys):
    from pyimgano.infer_cli import main as infer_main
    from pyimgano.train_cli import main as train_main
    ...
    MODEL_REGISTRY.register(
        "test_integration_workbench_manifest_dummy_detector_infer_config",
        _DummyDetector,
        tags=("vision",),
        overwrite=True,
    )
        ...
        json.dumps(
            {
                ...
                "model": {
                    "name": "test_integration_workbench_manifest_dummy_detector_infer_config",
                    "device": "cpu",
                    "pretrained": False,
                    "contamination": 0.1,
                },
    ...
    code = train_main(["--config", str(cfg_path), "--export-infer-config"])
    assert code == 0
    capsys.readouterr()

    infer_cfg = run_dir / "artifacts" / "infer_config.json"
    assert infer_cfg.exists()
    assert (run_dir / "checkpoints" / "bottle" / "model.pt").exists()
