...
class AlgorithmBenchmark:
        ...
        for algo_name, algo_config in self.algorithms.items():
            ...
            try:
                result = self._benchmark_single(
                    algo_name, algo_config, train_images, test_images, test_labels, verbose
                )
                self.results[algo_name] = result

            except Exception as e:
                logger.error("Failed to benchmark %s: %s", algo_name, e)
                self.results[algo_name] = {"error": str(e), "success": False}
    ...
    def print_summary(self) -> None:
        ...
        print("-" * 80)
        header = (
            f"{'Algorithm':<20} {'AUROC':>8} {'AP':>8} {'F1':>8} {'Precision':>10} {'Recall':>8}"
        )
        print(header)
        print("-" * 80)
        ...
        if successful:
            ...
            print(
                f"Fastest Training:    {fastest_train[0]} ({fastest_train[1]['train_time']:.2f}s)"
            )

            # Fastest inference
            fastest_inf = min(successful.items(), key=lambda x: x[1]["inference_per_image"])
            print(
                f"Fastest Inference:   {fastest_inf[0]} "
    ...
    def get_rankings(self, metric: str = "auroc") -> List[tuple]:
        ...
        if metric in ["auroc", "average_precision", "train_time", "inference_per_image"]:
            rankings = [(name, res[metric]) for name, res in successful.items()]
        elif metric in ["f1", "precision", "recall", "accuracy"]:
            rankings = [(name, res["metrics"][metric]) for name, res in successful.items()]
        else:
