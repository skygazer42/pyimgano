...
def _cross_check_model_card_manifest(
    ...
    *,
    ...
    manifest_error_count_before = len(errors)

    weights_norm = report.normalized.get("weights", {})
    if not isinstance(weights_norm, dict):
        weights_norm = {}
    ...
    if explicit_entry is not None:
        ...
        if matched_entry is None:
            errors.append(
                f"Model card references unknown manifest entry: {explicit_entry!r}"
            )
    else:
        model_resolved_path = _nonempty_str(assets["weights"].get("resolved_path", None))
        ...
        if matched_entry is None and len(manifest_report.entries) > 0:
            errors.append(
                "No weights manifest entry matches the model card weights asset."
            )
    ...
    if matched_entry is not None:
        ...
        manifest_asset["entry_resolved_path"] = _nonempty_str(
            matched_entry.get("resolved_path", None)
        )

        model_resolved = _nonempty_str(assets["weights"].get("resolved_path", None))
        ...
        if model_resolved is not None and manifest_entry_resolved is not None:
            if model_resolved != manifest_entry_resolved:
                errors.append(
        ...
        for field in ("source", "license"):
            card_value = _nonempty_str(weights_norm.get(field, None))
        ...
        deployment = report.normalized.get("deployment", {})
        deployment_runtime = (
            _nonempty_str(deployment.get("runtime", None))
            if isinstance(deployment, dict)
            else None
        )
        supported_runtimes = _manifest_runtimes(matched_entry)
        if deployment_runtime is not None and supported_runtimes:
            if deployment_runtime not in supported_runtimes:
                warnings.append(
...
def validate_model_card(
    ...
    *,
    ...
    if weights is None:
        ...
        weights_norm: dict[str, Any] = {}
        weights_asset: dict[str, Any] = {
            "path": None,
            "resolved_path": None,
            "exists": None,
            "sha256_match": None,
    ...
    if deployment is None:
        ...
        if cache_locations is not None:
            deployment_norm["cache_locations"] = _normalize_string_list(
                cache_locations, field_name="deployment.cache_locations", warnings=warnings
            )
