...
class PatchGridExtractor(BaseFeatureExtractor):
    ...
    def extract(self, inputs: Iterable[Any]) -> np.ndarray:
        ...
        self._ensure_ready()
        model = _require_initialized(self._model, owner="PatchGridExtractor", attribute="_model")
        transform = _require_initialized(
            self._transform, owner="PatchGridExtractor", attribute="_transform"
        )
        device = _require_initialized(self._device, owner="PatchGridExtractor", attribute="_device")
        torch = _require_initialized(self._torch, owner="PatchGridExtractor", attribute="_torch")
