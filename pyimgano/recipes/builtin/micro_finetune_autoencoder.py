...
from pyimgano.workbench.training_runtime import (
...
def micro_finetune_autoencoder(config: WorkbenchConfig) -> dict[str, Any]:
    ...
    tracker, tracker_meta = build_training_tracker(config=config, run_dir=paths.run_dir)
    ...
    if instrumentation_payload is not None:
        ...
        training["instrumentation"] = instrumentation_payload
